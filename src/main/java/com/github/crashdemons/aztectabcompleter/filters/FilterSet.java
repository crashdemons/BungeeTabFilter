...
import com.github.crashdemons.bungeetabfilter.BTFPlugin;
import com.github.crashdemons.bungeetabfilter.BasicPlugin;
import com.github.crashdemons.util.ConfigShim;
...
/**
 * Defines and controls a grouping of filter rules (whitelist/blacklist/group
 * equivalents) used by a plugin.
 * A filterset is a list of whitelist/blacklist rules defined by a given path in
 * the plugin config and requiring a given permission to be applicable.
 *
 * @author crash
 */
public class FilterSet {
     ...
     * @param pl the plugin
     */
    public FilterSet(BasicPlugin pl) {
    ...
    /**
     * Determines whether a user defined by the Filter arguments has the
     * permission relevant to this FilterSet
     *
    ...
    /**
     * Filters an input against the set of filters and decides action to take
     * from filtering.
     *
     * @param args the input filter argument pair (Player, Command name
     * suggestion) to check against filters
    ...
    /**
     * Filters an input against the set of filters and decides whether the input
     * should be allowed
     *
     * @param args the input filter argument pair (Player, Command name
     * suggestion) to check against filters
    ...
    private void loadDefaultResult(Configuration config) {
    ...
    private void loadLists(Configuration config, boolean logoutput) {
    ...
    private List<String> loadFilterOrder(Configuration config, boolean logoutput) {
        Configuration defaults = BTFPlugin.instance.getDefaultConfig();
        if ((new ConfigShim(config)).contains("filter-order", true)) {
            List<String> order = config.getStringList("filter-order");
            if (logoutput) {
                log("Loaded filter-order: " + order.toString());
            }
            return order;
        } else {
            if (logoutput) {
    ...
    private void loadGroups(Configuration config, boolean logoutput, List<String> filterOrder) {
        filterGroups.clear();
        ...
        Collection<String> groupnames = groups.getKeys();
        //if(logoutput) log(""+groupnames.size());
        //if(logoutput) log(groupnames.toString());
        for (String groupname : groupnames) {
            Configuration groupConfig = groups.getSection(groupname);
    ...
    /**
     * Load all filters, order, and groups from the config and enables them for
     * this set.
     *
     * @param config the configuration section to read for filter rules.
     */
    public void load(Configuration config) {//detect group load and don't load nested groups etc.
